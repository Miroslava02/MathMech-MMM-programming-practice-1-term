...
argument randomize(argument v, double T)  
{
	...
	for (int i = 0; i < D; i++)
	{
		do
		{
			...
			x_i_new = v.coord[i].val + b_i * (v.coord[i].right - v.coord[i].left);
		} while (x_i_new < v.coord[i].left || x_i_new > v.coord[i].right);
		temp.coord[i].val = x_i_new;
		temp.coord[i].left = v.coord[i].left;
		temp.coord[i].right = v.coord[i].right;
	}
...
void initialisation(argument& v_start)
{
	...
	for (int i = 0; i < D; i++)
	{
		cin >> v_start.coord[i].left >> v_start.coord[i].right;
		v_start.coord[i].val = v_start.coord[i].left + ((double)(rand()) / RAND_MAX) * (v_start.coord[i].right - v_start.coord[i].left);
	}
}
